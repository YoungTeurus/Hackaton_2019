...
void Map::gener(GameRoom* Curr, int deep)
{
	...
	if (Curr->get_i() > 0) {
		if (matrix[i - 1][j] == nullptr) {
			...
			matrix[i-1][j]->add_object(new GameObject(SDL_Point{ 200, 494 }, 100, 5, EXIT_DOWN));
	...
	if (Curr->get_j() - 1 < rooms) {
		if (matrix[i][j + 1] == nullptr) {
			...
			matrix[i][j + 1]->add_object(new GameObject(SDL_Point{ 1,200 }, 5, 100, EXIT_LEFT));
	...
	if (Curr->get_i() < rooms - 2) {
		if (matrix[i + 1][j] == nullptr) {
			...
			matrix[i + 1][j] = new GameRoom(i, j, room_template);
			matrix[i + 1][j]->add_object(new GameObject(SDL_Point{ 200, 1 }, 100, 5, EXIT_UP));
			matrix[i + 1][j]->set_i(i + 1);
			matrix[i + 1][j]->set_j(j);
			if (deepDelta > rand() % 100) {
				cout << "i: " << i << " j: " << j << endl;
	...
	if (Curr->get_j() > 0)
	{
		if (matrix[i][j - 1]) {
			...
			matrix[i][j - 1] = new GameRoom(i, j, room_template);
			matrix[i][j - 1]->add_object(new GameObject(SDL_Point{ 494, 200 }, 5, 100, EXIT_RIGHT));
			matrix[i][j - 1]->set_i(i);
			matrix[i][j - 1]->set_j(j - 1);
			if (deepDelta > rand() % 100) {
				cout << "i: " << i << " j: " << j << endl;
